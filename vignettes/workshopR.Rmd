---
title: "Workshop de R: Levando a Ciência de Dados para Todos"
author: "Marcus Nunes - https://marcusnunes.me/"
date: "18 e 19 de Julho de 2019"
...
- As quatro seguintes são opcionais: elas permitem que personalizemos nossas visualizações
  ...
  - tema: camada referente à visualização geral do gráfico

- Trabalharemos com o conjunto de dados `mtcars`
...
- Veja o código abaixo. Imagine o que ele fará antes de rodá-lo no `R`. O resultado esperado ocorreu? 
...
- Crie um gráfico de dispersão relacionando consumo na estrada com número de cilindros. O que é possível perceber?

```{r mpg_03, eval = FALSE}
# grafico de dispersao <???>
```

- Crie um gráfico de dispersão relacionando class versus drv. O que este gráfico está nos mostrando? Por que ele não é útil? 
...
- Note que somos capazes de gerar um gráfio mais sofisticado se alterarmos um pouco o código que utilizamos até o momento
...
- O que está de errado no gráfico abaixo? Por que os pontos não ficaram azuis? Como resolver este problema?
...
- Podemos dividir as variáveis de um conjunto de dados em dois grandes grupos: variáveis quantitativas (números) e variáveis categóricas (nomes). Quais variáveis do conjunto de dados `mpg` pertencem a cada grupo? Utilize `?mpg` e `str(mpg)` para tirar as suas conclusões.

- Repita o gráfico de dispersão de `hwy` versus `displ`, mas desta vez mapeie uma variável quantitativa para a cor. Qual é a diferença que podemos perceber?
...
- O que ocorre se uma variável contínua for utilizada para criar as facetas?
...
- Os exemplos com os quais trabalhamos inseriram a visualização de uma terceira variável em nossos gráficos utilizando cores e facetas. Qual destes dois métodos é mais indicado se a terceira variável for 
...
- Mas as não é necessário manter as separações em grupos iguais nas duas camadas; veja no códiga abaixo como separamos os pontos de acordo com a tração, mas mantivemos a tendência global da relação entre `hwy` e `displ` 
...
- Descubra o tipo de `geom` adequado para construir
  - Um gráfico de linha
  - Um boxplot
  - Um histograma

- Sem rodar, descreva o que o código abaixo fará. Rode-o em seguida para ver se a sua intuição estava correta.
...
- Escreva o código necessário para recriar os gráficos abaixo
...
- Utilize o comando `?theme_bw` para encontrar outros temas disponíveis

- Utilize o comando `?scale_colour_brewer` para encontrar outras paletas de cores disponíveis

- Crie uma visualização para o gráfico de dispersão de `hwy` versus `displ` com um tema e uma paleta de cores que ainda não foi vista
...
- Utilize a função `n()` para encontrar o fabricante de automóveis com mais modelos no conjunto de dados

- Determine quais são os fabricantes que comercializam carros com tração traseira

- Calcule as médias de consumo na estrada dos modelos de carros do ano de 1998 e mostre os cinco mais econômicos
...
- É natural criar um gráfico de barras com este resultado
...
- Perceba que não coneguimos identificar a variável `drv` neste gráfico

- Isto ocorre porque, por padrão, o `geom_col` fazer barras empilhadas
...
- Para remediar isso, utilizamos a função `complete`:
...
- Perceba que até o momento não nos precupamos em melhorar a comunicação dos nossos gráficos
...
- Além do conjunto `mtcars`, foi disponibilizado com este pacote um conjunto de dados chamado `gapminder`, com informações econômicas sobre vários países do mundo
...
- Um de seus co-fundadores, o estatístico Hans Rosling, ficou conhecido em 2010 ao apresentar na BBC um vídeo de 5 minutos demonstrando como a expectativa de vida dos países aumentou nos últimso 200 anos

- [Este vídeo possui mais de 8 milhões de visualizações apenas no YouTube](https://www.youtube.com/watch?v=jbkSRLYSojo)
...
- Falem com seus coordenadores de curso e peçam disciplinas para o Departamento de Estatística
